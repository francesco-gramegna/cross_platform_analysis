...
human <- c('Sports', 'Music',  'Beauty&Fashion') #for music


before <- df[df$X_year == "2022",]
before <- before[before$X_month == "march", ]

#before <- before[before$er_pct != 'None']
before <- before[order(-before$followers), ]
before <- before[1:100, ]

after <- df[df$X_year == "2024",]

after <- after[after$X_is_global == "True", ]

print(unique(after$er_pct))
